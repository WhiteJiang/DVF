...
import model.vit_config as vit_config
...
class Part_Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        last_map = torch.sum(last_map, dim=1, keepdim=True)
        # last_map = torch.prod(last_map, dim=1, keepdim=True)
        _, max_inx = torch.topk(last_map, dim=2, k=16)
        return _, max_inx
...
class AttentionModule(nn.Module):
    def __init__(self):
        ...
        self.attention_layer = nn.Linear(768, 1)
    ...
    def forward(self, x, features):
        last_map = x[-1]
        last_map = last_map[:, :, 0, 1:]
        cls_map = torch.sum(last_map, dim=1, keepdim=True)
        # cls_map = self.softmax(cls_map)
        attention_map = self.attention_layer(features)
        attention_map = self.activate(attention_map)
        # attention_map = self.softmax(attention_map)
        attention_map = attention_map.permute(0, 2, 1)
        final_map = cls_map * attention_map + cls_map
        # final_map = attention_map
        _, max_inx = torch.topk(final_map, dim=2, k=12)
        return _, max_inx
...
class Encoder(nn.Module):
    def __init__(self, config):
        ...
        self.part_select = AttentionModule()
    ...
    def forward(self, hidden_states):
        ...
        part_num, part_inx = self.part_select(attn_weights, hidden_states[:, 1:])
        temp_index = part_inx
        part_inx = part_inx + 1
        parts = []
        #
        B, _, _ = hidden_states.size()
        part_policy = torch.zeros(197 - 1)
        # print(part_inx.size())
        for i in range(B):
            part_policy[temp_index[i, :]] = 1
        for i in range(B):
        ...
        return part_encoded, part_policy
...
class Transformer(nn.Module):
    ...
    def forward(self, input_ids):
        ...
        part_encoded, part_policy = self.encoder(embedding_output)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, labels=None):
        part_tokens, part_policy = self.transformer(x)

        return part_tokens[:, 0], part_policy
...
CONFIGS = {
    ...
    'ViT-B_32': vit_config.get_b32_config(),
    'ViT-L_16': vit_config.get_l16_config(),
    'ViT-L_32': vit_config.get_l32_config(),
    'ViT-H_14': vit_config.get_h14_config(),
    'testing': vit_config.get_testing(),
}
